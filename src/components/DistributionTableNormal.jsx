export default function DistributionNormal({ variableName, result }) {
  const { computedValues, probabilityText, countMatching, probability } = result;

  return (
    <div className="p-4 bg-gray-800 text-white rounded-md">
      <h2 className="text-lg font-bold mb-4">Resultados - Distribución Normal</h2>

      <table className="w-full border-collapse border border-gray-600 mb-4">
        <thead>
          <tr className="bg-gray-700 text-center">
            <th className="border border-gray-600 p-2">Números Aleatorios (t₁, t₂)</th>
          ...
          {computedValues.map((row, index) => (
            <tr key={index} className="text-center">
              <td className="border border-gray-600 p-2">
                ({row.random[0].toFixed(4)}, {row.random[1].toFixed(4)})
              </td>
